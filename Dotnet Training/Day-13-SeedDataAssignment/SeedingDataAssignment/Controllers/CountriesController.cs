...
namespace SeedingDataAssignment.Controllers
{
    ...
    public class CountriesController : ControllerBase
    {
        ...
        public CountriesController(ContinentContext context)
        {
            _context = context;
        }
        ...
        public async Task<ActionResult<IEnumerable<Country>>> GetCountries()
        {
            ...
            var result = await _context.Countries
                .Select(c => new Country
                {
                    ...
                    States = c.States.Select(s => new State
                    {
                        StateId = s.StateId,
        ...
        {
            var country = await _context.Countries.Include(c => c.States).FirstOrDefaultAsync(c => c.CountryId == id);
            if (country == null)
            {
                return NotFound();
            }
            var result = new Country 
            {
                ...
                States = country.States.Select(s => new State
                {
                    StateId = s.StateId,
                    StateName = s.StateName,
                    CountryId = s.CountryId
